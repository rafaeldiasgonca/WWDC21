...
public extension Scene {
    ...
    func createBackgroundUI(background1: Color, background1Gradient: Color, background2: Color, background2Gradient: Color, dots: Color, displayGraphic: Image, displayGraphicName: String, dotHeight: Double = -100) {
        let titleBackground1 = Graphic(shape: RectangleShape(width: 800, height: 863,  cornerRadius: 0), color: background1, colors: [backgroundColor, background1Gradient], name: Display.title)
        titleBackground1.alpha = 0.95
        let titleBackground2 = Graphic(shape: RectangleShape(width: 800, height: 863,  cornerRadius: 0), color: background2, colors: [ background2, background2Gradient], name: Display.title)
        ...
        for _ in 1...11 {
            let dot = Graphic(shape: CircleShape(radius: 4), color: dots, name: Display.title)
            dot.alpha = 0.6
    ...
    @discardableResult func createPracticeButton(handler: @escaping (Touch) -> Void) -> Button {
    ...
    @discardableResult func createPlayButton(text: String, handler: @escaping (Touch) -> Void) -> Button {
    ...
    func createDisplayLabel(backgroundColor: Color, alpha: Double, position: Point, labelTitle: String, startingText: String = "", name: String = "displayLabel", backgroundName: String = "displayLabel", labelTextColor: Color = .white) -> Label {
        let background = Graphic(shape: RectangleShape(width: 200, height: 100, cornerRadius: 20), color: backgroundColor, name: backgroundName)
